...
class FramingEstimator:
    ...
    @staticmethod
    def calculate_stud_count(
            wall,
            doors: list = None,
            windows: list = None,
            connected_walls: int = 0) -> int:
        """
    ...
    @staticmethod
    def estimate_wall_materials(
            wall,
            doors: list = None,
            windows: list = None,
            connected_walls: int = 0) -> dict:
        """
        ...
        stud_count = FramingEstimator.calculate_stud_count(
            wall, doors, windows, connected_walls)
    ...
    @staticmethod
    def estimate_all_walls(
            wall_sets: list,
            walls_with_openings: dict = None) -> dict:
        """
        ...
        # missing)
        _cfg = config.load_config() if hasattr(
            config, "load_config") else getattr(
            config, "DEFAULT_SETTINGS", {})
        MAX_WALL_PLATE_INCHES = _cfg.get(
            "MAX_WALL_PLATE_INCHES", getattr(
                config, "DEFAULT_SETTINGS", {}).get(
        ...
        for wall_set in wall_sets:
            for wall in wall_set:
                ...
                # Get connected walls count (walls in same set + 1 for each
                # connection)
