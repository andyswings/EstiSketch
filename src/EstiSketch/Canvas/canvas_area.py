...
from ..components import Wall, Room, Text, Dimension, Layer, Level
...
class CanvasArea(Gtk.DrawingArea,
    ...
    def __init__(self, config_constants):
        ...
        # current segments (list of Polyline)
        self.polylines = []
        ...
        # List of door placements; each item is a tuple: (wall, door,
        # position_ratio)
        self.doors = []
        # List of window placements; each item is a tuple: (wall, window,
        # position_ratio)
        self.windows = []
    ...
    def _ensure_valid_active_layer(self):
        ...
        if active_layer and (
                not active_layer.level_id or active_layer.level_id == self.active_level_id):
            return
        ...
        from ..components import Layer
        # add_layer will need update or manual handling?
        new_id = self.add_layer(f"Layer 1")
        # Actually add_layer needs to know about current level.
        # For now, let's fix add_layer separately or handle it here?
        # Let's fix add_layer in the next chunk/step, but for now just fallback
        # to 0
        if self.layers:
            self.active_layer_id = self.layers[0].id
    ...
    def delete_selected(self):
        ...
        for item in list(self.selected_items):
            ...
            # Polylines: search and remove from polyline_sets (list of lists)
                ...
                selected_id = item.get("identifier") or getattr(
                    selected_obj, "identifier", None)
                ...
                for poly_list in list(self.polyline_sets):
                    ...
                    for idx, segment in enumerate(poly_list):
                        if segment is selected_obj or (
                            selected_id is not None and getattr(
                                segment, "identifier", None) == selected_id):
                            found_index = idx
                            break
        ...
        for room_id, indices in room_vertices_to_delete.items():
            ...
            target_room = next(
                (r for r in self.rooms if r.identifier == room_id), None)
            if not target_room:
                continue
    ...
    def select_all(self):
        ...
        for wall_set in self.wall_sets:
            for wall in wall_set:
                if not self.is_object_on_locked_layer(
                        wall) and self.is_object_on_visible_layer(wall):
        ...
        for room in self.rooms:
            if not self.is_object_on_locked_layer(
                    room) and self.is_object_on_visible_layer(room):
        ...
        for door_item in self.doors:
            ...
            if not self.is_object_on_locked_layer(
                    door_item[1]) and self.is_object_on_visible_layer(
                    door_item[1]):
                self.selected_items.append(
                    {"type": "door", "object": door_item})

        # Select all windows
        for window_item in self.windows:
            if not self.is_object_on_locked_layer(
                    window_item[1]) and self.is_object_on_visible_layer(
                    window_item[1]):
                self.selected_items.append(
        ...
        for text in self.texts:
            if not self.is_object_on_locked_layer(
                    text) and self.is_object_on_visible_layer(text):
                self.selected_items.append({"type": "text", "object": text})

        # Select all dimensions
        for dim in self.dimensions:
            if not self.is_object_on_locked_layer(
                    dim) and self.is_object_on_visible_layer(dim):
                self.selected_items.append(
    ...
    def paste(self):
        ...
        for item in self.clipboard:
            ...
            if item_type == "wall":
                ...
                new_wall = self.Wall(
                    start=(
                        wall.start[0] + offset[0],
                        wall.start[1] + offset[1]),
                    end=(
                        wall.end[0] + offset[0],
                        wall.end[1] + offset[1]),
                    width=wall.width,
                ...
                new_window.identifier = self.generate_identifier(
                ...
                self.selected_items.append(
                    {"type": "window", "object": new_window_tuple})
                ...
                new_text.identifier = self.generate_identifier(
                ...
                new_dim = self.Dimension(
                    start=(
                        dim.start[0] + offset[0],
                        dim.start[1] + offset[1]),
                    end=(
                        dim.end[0] + offset[0],
                        dim.end[1] + offset[1]),
                    offset=dim.offset,
                ...
                new_poly.start = (
                    poly.start[0] + offset[0],
                    poly.start[1] + offset[1])
                new_poly.end = (
                    poly.end[0] + offset[0],
                    poly.end[1] + offset[1])
                new_poly.identifier = self.generate_identifier(
                    "polyline", self.existing_ids)

                # Add as a new polyline set
                self.polyline_sets.append([new_poly])
                self.selected_items.append(
                    {"type": "polyline", "object": new_poly, "identifier": new_poly.identifier})
        ...
        for item in self.clipboard:
            if item["type"] == "vertex":
                ...
                # Use object id to track unique rooms
                rooms_to_paste.add(id(room))
        ...
        for item in self.clipboard:
            if item["type"] == "vertex":
                ...
                if id(room) in rooms_to_paste:
                    ...
                    new_points = [(pt[0] + offset[0], pt[1] + offset[1])
