...
class CanvasStateMixin:
    ...
    def restore_state(self, state):
        ...
        self.current_wall = copy.deepcopy(
            state["current_wall"]) if state["current_wall"] else None
        ...
        self.queue_draw()
        print(
            f"restore_state: {len(self.wall_sets)} wall sets, {len(self.walls)} walls, {len(self.rooms)} rooms")

    def undo(self):
        # We need at least 2 states: current state (to save) and previous state
        # (to restore)
