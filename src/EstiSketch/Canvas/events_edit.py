...
class EditEventsMixin:
    ...
    def separate_walls(self, popover: Gtk.Popover) -> None:
        ...
        """
        selected_walls = [item["object"]
                          for item in self.selected_items if item["type"] == "wall"]
        if not selected_walls:
            return

        affected_sets_indices = set()
        # Lists of walls (whole sets) that are not affected
        walls_to_keep_as_is = []
        ...
        # Separate each selected wall into its own set (do NOT regroup by
        # connectivity)
    ...
    def split_wall(self, popover: Gtk.Popover) -> None:
        ...
        w1 = self.Wall(
            wall.start, midpoint, wall.width, wall.height, getattr(
                wall, "exterior_wall", True), identifier=self.generate_identifier(
                "wall", self.existing_ids))
        # Wall 2: midpoint -> end
        w2 = self.Wall(
            midpoint, wall.end, wall.width, wall.height, getattr(
                wall, "exterior_wall", True), identifier=self.generate_identifier(
                "wall", self.existing_ids))
    ...
    def on_drag_update(
        ...
        if getattr(
            ...
            if self.snap_manager.snap_enabled:
                ...
                for wall_obj, endpoint_name in getattr(
                    ...
                    snap_pt, snap_type = self.snap_manager.snap_to_angle(
                        new_x, new_y, anchor[0], anchor[1])
        ...
        if getattr(
            ...
            current_device_x = self.drag_start_x + \
                offset_x if hasattr(self, 'drag_start_x') else offset_x
            current_device_y = self.drag_start_y + \
                offset_y if hasattr(self, 'drag_start_y') else offset_y

            # Convert to model coordinates
            new_x, new_y = self.device_to_model(
                current_device_x, current_device_y, pixels_per_inch)

            # Apply snapping if enabled
            if self.snap_manager.snap_enabled:
                walls = [
                    wall for wall_set in self.wall_sets for wall in wall_set]
                # Calculate standard snaps
                ...
                # Determine winner
                start_dist_sq = (new_x - snapped_pos[0])**2 + (new_y - snapped_pos[1])**2
                face_dist_sq = float('inf')
                if best_face_snap:
                    face_dist_sq = (new_x - best_face_snap[0])**2 + (new_y - best_face_snap[1])**2
                
                # If face snap is valid and closer (or if standard snap failed/was just 'grid' maybe?)
                # Actually, standard logic: pick closest.
                if best_face_snap and (face_dist_sq < start_dist_sq or snap_type == "none"):
        ...
        if getattr(self, "dragging_vertices", None):
            ...
            current_model = self.device_to_model(
                current_device_x, current_device_y, pixels_per_inch)
        ...
        # Handle dimension dragging - moves entire dimension (start and end
        # points together)
        ...
        # Handle polyline dragging - moves entire polyline (start and end
        # points together)
        ...
        if getattr(self, "dragging_room", None):
            ...
            for i, orig_pt in enumerate(self.dragging_room_original_points):
                self.dragging_room.points[i] = (
                    orig_pt[0] + dx, orig_pt[1] + dy)
        ...
        if getattr(
            ...
            if self.snap_manager.snap_enabled:
                walls = [
                    wall for wall_set in self.wall_sets for wall in wall_set]
                polylines = [
                    pl for poly_set in self.polyline_sets for pl in poly_set]
                snapped_pos, snap_type = self.snap_manager.snap_point(
                    new_x, new_y,
                    new_x, new_y,
                    walls, self.rooms,
        ...
        if getattr(self, "dragging_wall", None):
            ...
            new_start = (
                self.wall_drag_original_start[0] + dx,
                self.wall_drag_original_start[1] + dy)
            new_end = (
                self.wall_drag_original_end[0] + dx,
                self.wall_drag_original_end[1] + dy)
        ...
        if getattr(self, "dragging_door_window", None):
            ...
            # Calculate current device position using stored start pos + offset
            # (delta)
            current_device_x = self.drag_start_x + offset_x
            current_device_y = self.drag_start_y + offset_y

            # Convert to model coords using the helper function
            cursor_x, cursor_y = self.device_to_model(
                current_device_x, current_device_y, pixels_per_inch)
            ...
            for i, wall_set in enumerate(self.wall_sets):
                for j, check_wall in enumerate(wall_set):
                    dist = self.distance_point_to_segment(
                        (target_x, target_y), check_wall.start, check_wall.end)

                    # Qualification check: Current wall is always valid, others
                    # must be within threshold
                    is_current_wall = (check_wall is wall)
                    is_valid_candidate = is_current_wall or (
                        dist < snap_threshold)

                    if is_valid_candidate:
                        # Optimization check: Is this strictly closer than the
                        ...
                        if dist < best_dist:
                            ...
                            if wall_len_sq > 0:
                                dot = (
                                    target_x - check_wall.start[0]) * wx + (target_y - check_wall.start[1]) * wy
                                # Clamp to keep on wall
                                best_ratio = max(
                                    0.05, min(0.95, dot / wall_len_sq))
                            else:
                                best_ratio = 0.5
        ...
        if self.tool_mode == "panning":
            ...
            current_y = self.box_select_start[1] + \
                (offset_y / (self.zoom * pixels_per_inch))
            ...
            # Convert to model
            pixels_per_inch = getattr(self.config, "PIXELS_PER_INCH", 2.0)
            start_m_x, start_m_y = self.device_to_model(
                self.drag_start_x, self.drag_start_y, pixels_per_inch)
            curr_m_x, curr_m_y = self.device_to_model(
                current_x, current_y, pixels_per_inch)
