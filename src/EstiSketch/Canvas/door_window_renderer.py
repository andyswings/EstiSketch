    ...
    for door_item in self.doors:
        ...
        if door.door_type == "single":
            ...
            if door.orientation == "inswing":
                ...
                offset_vector = (
                    t / 2 * current_normal[0],
                    t / 2 * current_normal[1])
            ...
            # Draw the swing arc
            angle_closed = math.atan2(
                hinge_wall_direction[1],
                hinge_wall_direction[0])
            # Calculate the angle towards which the door opens (based on
            # current_normal)
            angle_open_direction = math.atan2(
                current_normal[1], current_normal[0])

            # Calculate the angle difference to determine sweep direction
            # (CW/CCW)
            ...
            # Determine the 90-degree sweep angle, preserving the sign for
            # direction
        ...
        if door.door_type == "double":
            ...
            hinge1 = (H_start[0] + (t / 2) * n_override[0],
                      H_start[1] + (t / 2) * n_override[1])  # Left hinge
            hinge2 = (H_end[0] + (t / 2) * n_override[0],
                      H_end[1] + (t / 2) * n_override[1])      # Right hinge
            ...
            if door.orientation == "outswing":
                # Left leaf: 90 degrees counter-clockwise from closed to open
                # (toward n)
                ...
                cr.arc_negative(
                    ...
                    angle_open_left)
                cr.stroke()
                # Right leaf: 90 degrees clockwise from opposite side to open
                # (toward n)
                ...
                # Left leaf: 90 degrees clockwise from closed to open (toward
                # -n)
                angle_open_left = angle_closed + math.pi / 2
                cr.arc(
                    hinge1[0],
                    ...
                    angle_open_left)
                cr.stroke()
                # Right leaf: 90 degrees counter-clockwise from opposite side
                # to open (toward -n)
                angle_open_right = angle_closed + math.pi / 2
                cr.new_path()
                cr.arc_negative(
        ...
        if door.door_type == "sliding":
            ...
            wall_start = (H_start[0] + (t / 2) * n[0],
                          H_start[1] + (t / 2) * n[1])  # Left edge (outside)
            wall_end = (H_end[0] + (t / 2) * n[0], H_end[1] +
                        (t / 2) * n[1])      # Right edge (outside)
            wall_inside_start = (
                H_start[0] - (t / 2) * n[0], H_start[1] - (t / 2) * n[1])  # Left inside
            wall_inside_end = (H_end[0] - (t / 2) * n[0],
                               H_end[1] - (t / 2) * n[1])      # Right inside
            ...
            # Door leaf width (half of door opening width, assuming they
            ...
            left_leaf_end = (
                ...
                wall_mid_start[1] +
                leaf_width *
                d[1])  # Ends halfway across
            left_top_start = (
                left_leaf_start[0] + offset * n[0],
                left_leaf_start[1] + offset * n[1])  # Top edge
            left_top_end = (
                left_leaf_end[0] + offset * n[0],
                left_leaf_end[1] + offset * n[1])
            left_bottom_start = (left_leaf_start[0] +
                                 (offset -
                                  leaf_thickness) *
                                 n[0], left_leaf_start[1] +
                                 (offset -
                                  leaf_thickness) *
                                 n[1])  # Bottom edge
            left_bottom_end = (left_leaf_end[0] +
                               (offset -
                                leaf_thickness) *
                               n[0], left_leaf_end[1] +
                               (offset -
            ...
            # Right leaf: Positioned to show right side open, offset below
            # centerline
            right_leaf_start = (
                wall_mid_end[0] -
                leaf_width *
                ...
                wall_mid_end[1] -
                leaf_width *
                d[1])  # Starts halfway, ends at right edge
            right_leaf_end = wall_mid_end  # Ends at right edge of opening
            right_top_start = (
                right_leaf_start[0] - offset * n[0],
                right_leaf_start[1] - offset * n[1])  # Top edge
            right_top_end = (
                right_leaf_end[0] - offset * n[0],
                right_leaf_end[1] - offset * n[1])
            right_bottom_start = (right_leaf_start[0] -
                                  (offset -
                                   leaf_thickness) *
                                  n[0], right_leaf_start[1] -
                                  (offset -
                                   leaf_thickness) *
                                  n[1])  # Bottom edge
            right_bottom_end = (right_leaf_end[0] -
                                (offset -
                                 leaf_thickness) *
                                n[0], right_leaf_end[1] -
                                (offset -
                                 leaf_thickness) *
                                n[1])
        ...
        if door.door_type == "pocket":
            ...
            # Door leaf position (centered in wall thickness, partially
            # protruding into opening)
            if door.swing == "left":
                # Slides left: most of door in left wall, protruding slightly
                # into opening
                door_start = (wall_mid_start[0] -
                              (door_width *
                               3 /
                               4) *
                              d[0], wall_mid_start[1] -
                              (door_width *
                               3 /
                               4) *
                              d[1])  # 2/3 into wall
                door_end = (wall_mid_start[0] +
                            (door_width /
                             4) *
                            d[0], wall_mid_start[1] +
                            (door_width /
                ...
                # Slides right: most of door in right wall, protruding slightly
                # into opening
                door_start = (wall_mid_end[0] -
                              (door_width /
                               4) *
                              d[0], wall_mid_end[1] -
                              (door_width /
            ...
            # Normal vector n is perpendicular to door direction d, so use it
            # for thickness offset
        ...
        if door.door_type == "bi-fold":
            ...
            # For a left-swing door, subtract 60°; for a right-swing door, add
            # 60°.
            if door.swing == "left":
        ...
        if door.door_type == "double bi-fold":
            ...
            # Hinge points for the left and right sides (using the overridden
            # n)
            hinge_start = (H_start[0] + (t / 2) * n[0],
                           H_start[1] + (t / 2) * n[1])  # Left hinge
            hinge_end = (H_end[0] + (t / 2) * n[0],
                         H_end[1] + (t / 2) * n[1])  # Right hinge

            # Define the basic angles
            angle_60 = math.pi / 3             # 60° in radians
            # Door closed is along the wall direction
            ...
            if door.orientation == "outswing":
                # 60° counter-clockwise from closed position
                angle_left1 = angle_closed - angle_60
                # Second panel folds 120° clockwise
                angle_left2 = angle_left1 + 2 * angle_60
            else:  # "inswing"
                # 60° clockwise from closed position
                angle_left1 = angle_closed + angle_60
                # Second panel folds 120° counter-clockwise
                angle_left2 = angle_left1 - 2 * angle_60
            ...
            # For "inswing": use the right inswing geometry: π offset then
            # subtract 60°.
            if door.orientation == "outswing":
                angle_right1 = angle_closed + math.pi + angle_60   # π offset then add 60°
                # Second panel folds 120° counter-clockwise
                angle_right2 = angle_right1 - 2 * angle_60
            else:  # "inswing"
        ...
        if door.door_type == "garage":
            ...
            start = (H_start[0] + (t / 2) * n[0],
                     H_start[1] + (t / 2) * n[1])  # Left edge
            end = (H_end[0] + (t / 2) * n[0],
                   H_end[1] + (t / 2) * n[1])      # Right edge

            # Inside wall endpoints (opposite side of wall thickness)
            inside_start = (H_start[0] - (t / 2) * n[0],
                            H_start[1] - (t / 2) * n[1])  # Left inside
            inside_end = (H_end[0] - (t / 2) * n[0],
                          H_end[1] - (t / 2) * n[1])      # Right inside

            depth = door.height  # Extend inward by the door's height

            # Endpoints of the inner dashed rectangle (extending inside the
            # garage)
            inner_left = (
                inside_start[0] - depth * n[0],
                inside_start[1] - depth * n[1])
            inner_right = (
                inside_end[0] - depth * n[0],
                inside_end[1] - depth * n[1])
        ...
        # Compute label position (offset from wall center H, opposite the swing
        # direction n)
        margin = 6.0  # Offset in inches
        t = 4.0       # Wall thickness in inches, example value
        label_pos = (H[0] - (t / 2 + margin) * n[0],
                     H[1] - (t / 2 + margin) * n[1])
...
def draw_windows(self, cr, pixels_per_inch):
    ...
    for window_item in self.windows:  # window_item = (wall, window, ratio)
        # wall is a Wall object, window is a Window object, ratio is a float
        ...
        P1 = (H_start[0] - (t / 2) * p[0], H_start[1] -
              (t / 2) * p[1])  # Bottom-left corner
        P2 = (H_start[0] + (t / 2) * p[0], H_start[1] +
              (t / 2) * p[1])  # Bottom-right corner
        P3 = (H_end[0] + (t / 2) * p[0], H_end[1] +
              (t / 2) * p[1])  # Top-right corner
        P4 = (H_end[0] - (t / 2) * p[0], H_end[1] -
              (t / 2) * p[1])  # Top-left corner
        ...
        if window.window_type == "sliding":
            ...
            # Wall endpoints with thickness offset
            wall_start = (H_start[0] + (t / 2) * p[0],
                          H_start[1] + (t / 2) * p[1])  # Left edge (outside)
            wall_end = (H_end[0] + (t / 2) * p[0], H_end[1] +
                        (t / 2) * p[1])      # Right edge (outside)
            wall_inside_start = (
                H_start[0] - (t / 2) * p[0], H_start[1] - (t / 2) * p[1])  # Left inside
            wall_inside_end = (H_end[0] - (t / 2) * p[0],
                               H_end[1] - (t / 2) * p[1])      # Right inside
            ...
            # Window pane width (half of door opening width, assuming they
            # overlap slightly when closed)
            pane_width = w / 2
            ...
            left_pane_end = (
                ...
                d[1])  # Ends halfway across
            left_top_start = (
                left_pane_start[0] + offset * p[0],
                left_pane_start[1] + offset * p[1])  # Top edge
            left_top_end = (
                left_pane_end[0] + offset * p[0],
                left_pane_end[1] + offset * p[1])
            left_bottom_start = (left_pane_start[0] +
                                 (offset -
                                  pane_thickness) *
                                 p[0], left_pane_start[1] +
                                 (offset -
                                  pane_thickness) *
                                 p[1])  # Bottom edge
            left_bottom_end = (left_pane_end[0] +
                               (offset -
                                pane_thickness) *
                               p[0], left_pane_end[1] +
                               (offset -
            ...
            # Right pane: Positioned to show right side open, offset below
            # centerline
            right_pane_start = (
                wall_mid_end[0] -
                pane_width *
            ...
            right_top_start = (
                right_pane_start[0] - offset * p[0],
                right_pane_start[1] - offset * p[1])  # Top edge
            right_top_end = (
                right_pane_end[0] - offset * p[0],
                right_pane_end[1] - offset * p[1])
            right_bottom_start = (right_pane_start[0] -
                                  (offset -
                                   pane_thickness) *
                                  p[0], right_pane_start[1] -
                                  (offset -
                                   pane_thickness) *
                                  p[1])  # Bottom edge
            right_bottom_end = (right_pane_end[0] -
                                (offset -
                                 pane_thickness) *
                                p[0], right_pane_end[1] -
                                (offset -
                                 pane_thickness) *
                                p[1])
        ...
        if window.window_type == "double-hung":
            ...
            # Compute a unit vector along the window's width (from H_start to
            # H_end)
            ...
            rect_left_start = (
                wall_start[0] - extension * v[0],
                wall_start[1] - extension * v[1])
            rect_right_start = (
                wall_end[0] + extension * v[0],
                wall_end[1] + extension * v[1])

            # Now, push that line outward by one inch (using the outward vector
            # p)
            rect_left_outer = (
                rect_left_start[0] + extension * p[0],
                rect_left_start[1] + extension * p[1])
            rect_right_outer = (
                rect_right_start[0] + extension * p[0],
                rect_right_start[1] + extension * p[1])

            # Draw the extension rectangle outline
            # Black outline (or choose a different color/style if desired)
            cr.set_source_rgba(0, 0, 0, opacity)
