...
class CanvasEventsMixin:
            ...
            if item.get("type") == "wall":
                ...
                for handle_name, pt in [
                    ...
                    if math.hypot(dx, dy) < self.handle_radius:
                        ...
                        # Find ALL endpoints that share this joint (within
                        # tolerance)
                        connected = []
                        tol = getattr(
                            self.config, "JOINT_SNAP_TOLERANCE", 0.25)
        ...
        if hasattr(self, "selected_items"):
            for item in self.selected_items:
                if item["type"] == "text":
                    ...
                    text_x_dev, text_y_dev = self.model_to_device(
                        text.x, text.y, pixels_per_inch)

                    # Rotation handle is at top-right of text, rotated with
                    # text
                    ...
                    if math.hypot(dx, dy) < handle_radius:
                        ...
                        self.rotation_start_mouse_angle = math.degrees(
                            math.atan2(y - text_y_dev, x - text_x_dev))
                        return

                    # Otherwise, start moving the text
                    self.moving_text = text
                    self.moving_text_start_pos = (
                        self.moving_text.x, self.moving_text.y)
                    return
    ...
    def on_motion(
        ...
        if self.tool_mode == "draw_walls" and self.drawing_wall and self.current_wall:
            ...
            aligned_x, aligned_y, candidate = self._apply_alignment_snapping(
                canvas_x, canvas_y)
        ...
        if self.tool_mode == "add_polyline" and self.drawing_polyline:
            ...
            aligned_x, aligned_y, _ = self._apply_alignment_snapping(
                canvas_x, canvas_y)
            snapped_x, snapped_y = aligned_x, aligned_y
